...
poi_fix AS (
  SELECT
    ...
    CASE
      WHEN poi_geom_4326 IS NULL OR ST_IsEmpty(poi_geom_4326) THEN NULL
      WHEN ST_IsValid(poi_geom_4326) THEN poi_geom_4326
...
poi_h3 AS (
  SELECT
    ...
    CAST({{ h3_r7_from_geom_point('poi_centroid_4326') }} AS STRING) AS h3_r7
  FROM poi_fix
  WHERE poi_geom_4326 IS NOT NULL
    AND ST_IsValid(poi_geom_4326)
    AND NOT ST_IsEmpty(poi_geom_4326)
    AND poi_centroid_4326 IS NOT NULL
),
